        ...
        for (int fk = 0; fk < Nt_f; fk++) {
            for (int fj = 0; fj < Nt_f; fj++) {
                for (int fi = 0; fi < Nt_f; fi++) {
                    ...
                    /* Check nblevel: skip if same-level or finer neighbor
                     * filled this direction. Prolongation is ONLY for cells
                     * facing COARSER neighbors. Domain boundary ghost cells
                     ...
                     * Ref: Athena++ paper (Stone 2020) — "use finer
                     * MeshBlock values when available". */
                    int nlev = b->nblevel[oz + 1][oy + 1][ox + 1];
                    if (nlev >= b->loc.level) continue;  /* same-level or finer */
