    ...
    for (int k = 0; k < g->Ntotal; k++) {
        for (int j = 0; j < g->Ntotal; j++) {
            for (int i = 0; i < g->Ntotal; i++) {
                ...
                /* Conformal factor: psi = 1 + sum(M_n / 2r_n) */
                double psi = 1.0;
                for (int n = 0; n < n_bh; n++) {
                    ...
                    if (r < 1.0e-10) r = 1.0e-10; /* regularize */
                    psi += masses[n] / (2.0 * r);
                }

                double chi = 1.0 / (psi * psi * psi * psi); /* chi = psi^{-4} */

                /* Conformal metric = flat identity */
                ...
                /* Extrinsic curvature = 0 for time-symmetric data */
                g->fields[FIELD_K][idx]     = 0.0;
                g->fields[FIELD_A11][idx]   = 0.0;
                g->fields[FIELD_A12][idx]   = 0.0;
                g->fields[FIELD_A13][idx]   = 0.0;
                ...
                /* Pre-collapsed lapse for punctures */
