                              ...
                              int verbose, const char *tag)
{
    ...
    /* CPU composite solver for all backends.  FMG converges in ~1 pass
     * for puncture data, so the solver is memory-bound on ghost exchange.
     * CPU direct memcpy between blocks beats GPU kernel launch overhead
     * at all practical block counts (~1800 for 11-level inspiral).
     * Solver runs once at t=0; evolution uses GPU for time-stepping. */
